...
| 平台 | 登录 | 后续流程 | 当前状态 |
| --- | --- | --- | --- |
| 淘宝 | 二维码图片、短信验证、独立浏览器兜底 |商品校验、预约及抢购 | 已接入 |
| 京东 | 本地 Cookie 复用、在线校验、二维码重新登录 | 京东商品校验、预约及抢购 | 已接入 |
| 大麦 | 二维码图片、短信验证、独立浏览器兜底 | 暂不执行抢票 | 登录已接入，抢票开发中 |

其他已实现能力：

- 淘宝、京东、大麦 Cookie 分平台保存，过期或服务端失效后自动重新登录。
- Cookie 文件采用临时文件替换，降低程序中断造成文件损坏的风险。
- 淘宝和大麦二维码保存为本地图片，无需在无头浏览器窗口中扫码。
...
- Windows 10/11
- Python 3.11（源码运行）
- Chrome 或 Microsoft Edge（淘宝、大麦扫码登录及安全验证）
...
完成真实淘宝登录后，程序会继续取得有效的京东登录状态，再进入现有京东商品预约和抢购流程。淘宝 Cookie 不会被传入京东接口。

### 选择京东

程序优先复用 `cookies/jd.json`，登录状态失效时重新生成京东二维码。认证成功后进入京东预约和抢购流程。

### 选择大麦

程序优先复用 `cookies/damai.json`，失效时重新扫码。登录成功后显示真实用户名和“大麦抢票功能正在开发中”，不会调用京东接口，也不会模拟大麦抢票结果。
...
这些文件、二维码、验证截图和浏览器临时资料已加入 `.gitignore`。请勿在日志、Issue、聊天记录或提交中公开 Cookie、短信验证码、卡密、设备标识和抓包数据。
...
- 新增统一平台菜单：淘宝、京东、大麦。
- 新增淘宝和大麦真实二维码图片登录、短信二次验证及浏览器兜底。
- 新增三平台 Cookie 独立存储、过期检查和在线登录态校验。
- 淘宝登录后衔接有效京东会话；大麦登录成功后明确提示抢票开发中。
